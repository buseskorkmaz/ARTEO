...
prev_obj = power(refs - Calc_current1(safe_seed1) - Calc_current2(safe_seed2),2);
prev_constraint = limit - Calc_current1(safe_seed1)-Calc_current2(safe_seed2);

safe_ucb_obj = fitrgp([refs,safe_seed1,safe_seed2],  prev_obj, Standardize=true);
safe_ucb_constraint = fitrgp([safe_seed1,safe_seed2], prev_constraint);
...
prev_current_one_ucb = Calc_current1(safe_seed1);
prev_current_two_ucb = Calc_current2(safe_seed2);

prev_torque_one_ucb = safe_seed1;
prev_torque_two_ucb = safe_seed2;
...
%     end
    ...
    safe_ucb_regret_one = [safe_ucb_regret_one; abs(optimal-Calc_current1(x1)-Calc_current2(x2))];
...
stairs([1:i]', prev_current_one_ucb(3:i+2) + prev_current_two_ucb(3:i+2), ":", LineWidth=2, Color = "#005aff")
...
figure;
set(0,'DefaultLineLineWidth',2)
stairs([1:i]', predicted_one+predicted_two, Color="#00ffff", LineWidth=2);
hold
stairs([1:i]', prev_current_one_arteo(3:i+2)+prev_current_two_arteo(3:i+2), "--",LineWidth=2)
...
figure;
set(0,'DefaultLineLineWidth',2)
plot([1:i]', cumsum(arteo_avg_regret_one_z25(1:i)), Color="#ff748c", LineWidth=2)
hold
plot([1:i]', cumsum(arteo_avg_regret_one_z50(1:i)), Color="#ff0000", LineWidth=2)
plot([1:i]', cumsum(arteo_avg_regret_one_z100(1:i)), Color="#ab340f", LineWidth=2)
% plot([1:i]', cumsum(arteo_avg_regret_one(1:i)), Color="#ff900f", LineWidth=2)
% plot([1:i]', cumsum(safe_ucb_regret_one(1:i)),  Color="#005aff", LineWidth=2)
xlim([1 i])
xlabel("Time")
ylabel("Cumulative regret")
% legend("ARTEO regret (z=25)","Safe-UCB regret")
legend("ARTEO regret (z=25)","ARTEO regret (z=50)","ARTEO regret (z=100)")
